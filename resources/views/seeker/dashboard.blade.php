            ...
            <div class="filter">
                <form action="{{ route('dashboard') }}" method="get">
                    <select name="location">
                        <option value="{{ old('location') }}">Choose Location</option>
                        @foreach($location as $local)
                            <option value="{{ $local }}">{{ $local }} </option>
                        @endforeach
                    </select><br>
                    <select name="company">
                        <option value="{{ old('company') }}"> Choose Company</option>
                        @foreach($company as $comp)
                            <option value="{{ $comp->company_name }} ">{{ $comp->company_name }} </option>
                        @endforeach
                    </select><br>
                    <select name="type">
                        <option value="{{ old('type') }}">Choose Job Type</option>
                        @foreach($type as $typ)
                            <option value="{{ $typ->id }}">{{ $typ->name }} </option>
                        @endforeach
                    </select><br>
                    <select name="skill">
                        <option value="{{ old('skill') }}">Choose Skills</option>
